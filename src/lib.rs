...
pub fn compress<'a>(
  ...
  match input {
    Either::A(_) => Ok(Either::A(
      BufferSlice::from_data(env, buf.as_slice())
        .map_err(|e| Error::new(GenericFailure, format!("copy from failed: {}", e)))?,
    )),
...
pub fn uncompress<'a>(
  ...
  if let Some(max_length) = max_length {
    let expected_len = decompress_len(buf)
      .map_err(|e| Error::new(InvalidArg, format!("get uncompressed length failed: {}", e)))?;
    if expected_len as i64 > max_length {
      return Err(Error::new(
        GenericFailure,
        format!(
          "The uncompressed length of {expected_len} is too big, expect at most {max_length}"
        ),
      ));
    }
  }
